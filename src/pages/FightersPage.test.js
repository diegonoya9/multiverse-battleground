...
describe('FightersPage Component', () => {
    beforeEach(() => {
        // Limpia el mock de fetch antes de cada prueba
        global.fetch = jest.fn(); // Reemplaza global.fetch con un nuevo objeto jest.fn()
    ...
    it('renders correctly after fetching data and View Movements click works', async () => {
        ...
        const viewMovements = screen.getAllByText(i18n.t('fighterspage.viewMovements'))
        fireEvent.click(viewMovements[0])
        const move = screen.getByText('Flame')
        fireEvent.click(move)
    });
    it('renders correctly after adding to party', async () => {
        // Simula el resultado del fetch
        //const fakeData = [{ fighterId: 1, name: 'Fighter1' }];
        const mockJsonPromise = Promise.resolve(fakeData);
        const mockFetchPromise = Promise.resolve({
        ...
        const addToParty = screen.getAllByText(i18n.t('fighterspage.addToParty'))
        const fakeResponse = { ok:true };
        global.fetch.mockResolvedValueOnce({
          json: jest.fn().mockResolvedValueOnce(fakeResponse),
          ok: true,
        });
        fireEvent.click(addToParty[0])   
    
        // Espera a que se resuelva la llamada fetch después de hacer clic
        await waitFor(() => expect(global.fetch).toHaveBeenCalledTimes(1));
    
    });
    it('renders correctly after removing from party', async () => {
        const mockJsonPromise = Promise.resolve(fakeData);
        const mockFetchPromise = Promise.resolve({
            json: jest.fn().mockResolvedValueOnce(mockJsonPromise),
            ok: true,
        ...
        const removeFromParty = screen.getAllByText(i18n.t('fighterspage.removeFromParty'))
        const fakeResponse = { ok:true };
        global.fetch.mockResolvedValueOnce({
          json: jest.fn().mockResolvedValueOnce(fakeResponse),
          ok: true,
