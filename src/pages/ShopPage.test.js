...
describe('ShopPage Component', () => {
  ...
  test('renders ShopPage component', async () => {
   /* const mockJsonPromiseUser = Promise.resolve(user);
    const mockFetchPromiseUser = Promise.resolve({
        json: jest.fn().mockResolvedValueOnce(mockJsonPromiseUser),
        ok: true,
    });
    global.fetch = jest.fn().mockResolvedValueOnce(mockFetchPromiseUser);
    const mockJsonPromiseObjects = Promise.resolve(objects);
    const mockFetchPromiseObjects = Promise.resolve({
        json: jest.fn().mockResolvedValueOnce(mockJsonPromiseObjects),
        ok: true,
    });
    global.fetch = jest.fn().mockResolvedValueOnce(mockFetchPromiseObjects);
    const mockJsonPromiseFighters = Promise.resolve(fighters);
    const mockFetchPromiseFighters = Promise.resolve({
        json: jest.fn().mockResolvedValueOnce(mockJsonPromiseFighters),
        ok: true,
    });
    global.fetch = jest.fn().mockResolvedValueOnce(mockFetchPromiseFighters);*/
    await act(async () => {
        render(
